...
public interface CORCollectionBranchRepository extends JpaRepository<CORCollectionBranch, Long> {
	
	
	@Query(value = "SELECT * FROM vw_colbranch where branch_code = :branch_code ORDER BY id", nativeQuery = true )
	public List<CORCollectionBranch> getBranchCollection(@Param("branch_code") String branch); // , @Param("cash_office_code") String office);
...
//	public List<Object> getUnallocatedCashReceipts(@Param("start_date") String start_date, @Param("end_date") String end_date) ;
